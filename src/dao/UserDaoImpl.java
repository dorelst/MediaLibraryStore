...
public class UserDaoImpl implements UserDao {
	...
	private MediaItemDao mediaItemDao = new MediaItemDaoImpl();
	...
	public List<User> getUsersInfo() {		 
		...
		try {
			...
			PreparedStatement statement = connection.prepareStatement("SELECT * FROM user ORDER BY last_name ASC, first_name ASC, id ASC");
	...
	public List<User> getClassroomMembers(int classroomId) {
		try {
			PreparedStatement statement = connection.prepareStatement("SELECT * FROM user WHERE id IN (SELECT user_id FROM classroom_member WHERE classroom_id = ?) ORDER BY group_id DESC, last_name ASC, first_name ASC, id ASC");
	...
	public List<User> getUsersCurrentlyBookingMaterials() {
		try {
			PreparedStatement statement = connection.prepareStatement("SELECT * FROM user WHERE id IN (SELECT DISTINCT user_id FROM booked_item WHERE currently_booked = ?) ORDER BY last_name ASC, first_name ASC, group_id ASC, id ASC");
	...
	public Boolean addUser(User user) {
		...
		try {
			PreparedStatement statement = connection.prepareStatement("INSERT INTO user (group_id, first_name, last_name, username, password) VALUES (?, ?, ?, ?, ?)");
